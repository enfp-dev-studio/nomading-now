...
### Phase 1: 핵심 MVP (1-2주)
1. ✅ **S3 + 썸네일 + EXIF GPS** (6-8시간) - 🔴 HIGH
   - 이미지 업로드 인프라 구축
   - 사진에서 GPS 추출
   - 3가지 위치 소스 (현재/사진/수동)

2. ✅ **GPS 위치 검증** (4-6시간) - 🔴 HIGH
   - 현재 위치: 50m 검증
   - 사진 위치: 경고만
   - Haversine 거리 계산

3. ✅ **텍스트/이미지 제한** (1시간) - 🔴 HIGH
   - 280자 제한
   - 이미지 3장 제한
...
|-----|---------|---------|-------|------|
| S3 이미지 스토리지 + 썸네일 | 🔴 HIGH | 6-8h | 1 | 신규, EXIF GPS 포함 |
| 사진 EXIF GPS 활용 | 🔴 HIGH | 4-6h | 1 | 신규, 3가지 위치 소스 |
| GPS 위치 검증 | 🔴 HIGH | 2-3h | 1 | 기존, 50m 검증 |
| 텍스트 280자 제한 | 🔴 HIGH | 30m | 1 | 기존 |
| 이미지 3장 제한 | 🔴 HIGH | 30m | 1 | 기존 |
